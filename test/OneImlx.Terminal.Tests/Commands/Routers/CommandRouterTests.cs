...
namespace OneImlx.Terminal.Commands.Routers
{
    public class CommandRouterTests : IAsyncLifetime
    {
        ...
        public async Task ExtractorErrorShouldNotRouteFurtherAsync()
        {
            ...
            CommandContext commandContext = commandContextFactory.Create<CommandContext>(new(Guid.NewGuid().ToString(), "test_command_string"), routerContext, []);
        ...
        public async Task ExtractorNoExtractedCommandDescriptorShouldNotRouteFurtherAsync()
        {
            ...
            Func<Task> act = async () => await commandRouter.RouteCommandAsync(commandContext);
        ...
        public async Task ParserFails_But_Invokes_Handler()
        {
            ...
            commandContext.TryGetParsedCommand(out _).Should().BeFalse();
            await commandRouter.RouteCommandAsync(commandContext);
            commandContext.TryGetParsedCommand(out ParsedCommand? parsedCommand).Should().BeTrue();
        ...
        public async Task Router_Calls_Parser_And_Handler()
        {
            ...
            commandParser.Called.Should().BeTrue();
            commandParser.PassedContext.Should().BeSameAs(commandContext);

            commandHandler.Called.Should().BeTrue();
            commandHandler.PassedContext.Should().BeSameAs(commandContext);
        }
        ...
        public async Task Router_Passes_Updated_Context_From_Parser_To_Handler()
        {
            ...
            commandHandler.PassedContext.GetParsedCommand().Command.Descriptor.Id.Should().Be("test_id");
            commandHandler.PassedContext.GetParsedCommand().Command.Descriptor.Name.Should().Be("test_name");
        }
